    ...
    class Program
    {
       ...
       new ManagementObjectSearcher("root\\CIMV2",
        ...
        public static void WriteValueToReestr(string s) //Write value
        {
            ...
            RegistryKey myKey = currentUserKey.CreateSubKey("Androshchuk");
            myKey.SetValue("AndroshchukYurii", s);
            myKey.Close();
        ...
        {
            RegistryKey currentUserKey = Registry.CurrentUser;
            RegistryKey helloKey = currentUserKey.OpenSubKey("Androshchuk", true);
            // удаляем значение из ключа
            helloKey.DeleteValue("AndroshchukYurii");
            // удаляем сам ключ
            currentUserKey.DeleteSubKey("Androshchuk");
